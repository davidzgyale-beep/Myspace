...
with method_tab:
    ...
    with trend_col.container(border=True):
        ...
        st.markdown(
            """
            趋势分是固定公式的0–100分，理论满分为100，不是未来收益概率。收益率先分别计算全310只股票和所属子行业内的百分位，再按固定权重合成。120日收益率仍用于长期观察，但不参与趋势评分：
            ...
            三个收益窗口合计贡献90分：全股票池排名合计65分，子行业内排名合计25分；均线与高点确认合计10分。该公式不会再按当日分数分布做二次映射。
            """
        )
        st.code(
            "趋势分 = 65 x 全市场收益排名组合\n"
