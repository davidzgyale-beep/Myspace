...
第二批候选因子包括沪深300/医疗板块Beta、涨停与极端上涨、跳空与极端下跌、拥挤度季度变化和日内振幅/ATR。市场Beta使用沪深300（000300.SH），医疗Beta使用当前医疗股票池等权收益。当前5日与20日增强模型通过升级门槛；60日与120日因独立调仓期不足，继续使用基础模型。
...
看板只保留两个判断维度：趋势分，以及正式20日回撤风险模型产生的过热分。过热分是预测未来20日最大回撤风险的横截面百分位排名，分数越高表示相对风险越高。当前20日正式模型为已通过升级标准的增强回撤模型。
...
`backtest_groups.py` 已改为重建当前新口径。每个历史建仓日使用当时价格重算趋势分，并且只使用该日前已经完成20日持有期的历史样本滚动重训增强回撤模型，避免用今天的模型回填历史。过热分在当日310只股票中按风险预测值转成百分位。
...
- `data/two_dimension_backtest.csv`：趋势三档（强≥70、中40–70、弱<40）与风险三档（低<30、中30–70、高≥70）交叉后的5/20/120日平均收益、平均建仓后回撤、平均股票数和有效调仓期数。
