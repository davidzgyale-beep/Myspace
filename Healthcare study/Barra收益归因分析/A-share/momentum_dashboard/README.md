...
运行独立的市场状态与双Beta收益模型研究：

```bash
python return_model_research.py --refresh-benchmarks
```

收益模型以动态申万医药生物历史成员为股票池，主基准为 `000808.CSI` 中证申万医药生物指数，预测5日/20日个股相对收益。对照包含趋势基线、状态+双Beta、交互项，以及进一步加入Beta变化与残差特征的完整模型。`801150.SI` 申万行业指数日行情因Tushare权限无法读取；动态成分等权指数与 `159938.SZ` 中证全指医药卫生ETF仅用于稳健性对照。
...
看板使用趋势、实验性收益模型、正式20日回撤风险模型，以及市场状态与Beta情景层。收益模型仅用于相对研究排名，不改变趋势分、风险分或ABC分组。风险模型目标为未来20日最大不利波动（MAE）：`max(0, -min(未来持有期收益路径))`。价格在持有期从未跌破建仓价时MAE记为0。风险分是MAE预测值在310只股票中的横截面百分位排名，分数越高表示相对风险越高。
